...
| K13.10 | Masaüstü ilk kurulumu **tek komutla** yapılabilir: `GET /masaustu/uzaktan-kur.sh`; betik origin adaylarını (alan adı → genel IP) deneyip güncel paketi indirir, `~/.local/share/kutuphane-masaustu` içine açıp `kur.sh` çalıştırır. `MASAUSTU_ORIGIN` ile adres zorlanabilir. | `masaustu/uzaktan-kur.sh`, `masaustu/yukle_masaustu.sh` |
| K13.11 | Mobil + masaüstü açılışta sunucu **adaylarını paralel yoklar** (önce alan adı, sonra genel IP). Kayıtlı adres çalışıyorsa korunur; yoksa/çökmüşse ilk ulaşan ayarlanıp kaydedilir; hiçbiri yoksa kullanıcıya sunucu adresi sorulur. Kullanıcının elle girdiği özel adres otomatik değiştirilmez. Alan adı + SSL aktif olunca uygulama güncellenmeden otomatik geçer. | `mobil/.../sunucu_adresi.dart`, `masaustu/lib/sunucu_adresi.dart` |

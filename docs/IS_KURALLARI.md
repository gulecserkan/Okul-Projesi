...
| K9.7 | Uygulama ayrışımı: **masaüstü** yalnız **personel/admin** (`tip=personel`, üye girişi reddedilir); **mobil** yalnız **üye** (`tip=uye`, personel/admin girişi reddedilir). Mobil: `uye` → gezinti + ödünçlerim; `editor` → kitap düzenleme + ödünçlerim; ilk girişte şifre ekranı | giriş ekranları (masaüstü `AuthApi.login`, mobil `LoginScreen`) |

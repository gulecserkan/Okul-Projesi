...
class EncryptionTests(TestCase):
    def test_sensitive_fields_encrypted_at_rest(self):
        ...
        with connection.cursor() as cur:
            cur.execute("SELECT telefon, eposta FROM kutuphane_app_uye WHERE id=%s", [o.id])
    ...
    def test_legacy_plaintext_readable(self):
        ...
        rol = Rol.objects.create(ad="Öğrenci")
        o = Uye.objects.create(
            ad="Ayşe", soyad="Yılmaz", uye_no="T002", sinif=sinif, rol=rol
        )
        with connection.cursor() as cur:
            cur.execute(
                "UPDATE kutuphane_app_uye SET telefon=%s WHERE id=%s",
                ["05551234567", o.id],
            )
        o.refresh_from_db()
...
class CheckoutAPITests(APITestCase):
    ...
    def test_successful_checkout(self):
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60123", "barkod": "KIT0000999"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
        self.nusha.refresh_from_db()
        self.assertEqual(self.nusha.durum, "oduncte")
        self.assertTrue(OduncKaydi.objects.filter(uye=self.uye).exists())

    def test_duplicate_checkout_rejected(self):
        self.client.post("/api/checkout/", {"uye_no": "60123", "barkod": "KIT0000999"}, format="json")
        resp = self.client.post("/api/checkout/", {"uye_no": "60123", "barkod": "KIT0000999"}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_unknown_student_not_found(self):
        resp = self.client.post("/api/checkout/", {"uye_no": "YOK999", "barkod": "KIT0000999"}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)

    def test_pasif_student_rejected(self):
        self.uye.aktif = False
        self.uye.save()
        resp = self.client.post("/api/checkout/", {"uye_no": "60123", "barkod": "KIT0000999"}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.nusha.refresh_from_db()
    ...
    def test_max_items_reached(self):
        ...
        resp = self.client.post("/api/checkout/", {"uye_no": "60123", "barkod": nusha2.barkod}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        _ = kitap2
        _ = policy
...
class SearchFilterAPITests(APITestCase):
    """Uye/Kitap listelerinde q arama filtresi (Türkçe karakter duyarlı)."""
    ...
    def setUp(self):
        ...
        self.rol = Rol.objects.create(ad="Öğrenci")
        self.uye = Uye.objects.create(
            ad="İlber", soyad="Ortaylı", uye_no="5A01", sinif=self.sinif, rol=self.rol
        )
        Uye.objects.create(
            ad="Zeynep", soyad="Kaya", uye_no="5A02", sinif=self.sinif, rol=self.rol
        )
    ...
    def test_uye_q_turkce_dot_duyarli(self):
        # küçük i → büyük İ ("ilber" ↔ "İlber")
        resp = self.client.get("/api/uyeler/", {"q": "ilber"})
        self.assertEqual([o["uye_no"] for o in self._as_list(resp.data)], ["5A01"])

        resp = self.client.get("/api/uyeler/", {"q": "İlber"})
        self.assertEqual([o["uye_no"] for o in self._as_list(resp.data)], ["5A01"])

        # büyük I alt okuduğu → ı ("ORTAYLI" ↔ "Ortaylı")
        resp = self.client.get("/api/uyeler/", {"q": "ORTAYLI"})
        self.assertEqual([o["uye_no"] for o in self._as_list(resp.data)], ["5A01"])

        resp = self.client.get("/api/uyeler/", {"q": "ortayl"})
        self.assertEqual([o["uye_no"] for o in self._as_list(resp.data)], ["5A01"])

        resp = self.client.get("/api/uyeler/", {"q": "kaya"})
        self.assertEqual([o["uye_no"] for o in self._as_list(resp.data)], ["5A02"])

        resp = self.client.get("/api/uyeler/", {"q": "5-A"})
        self.assertEqual(len(self._as_list(resp.data)), 2)

        resp = self.client.get("/api/uyeler/", {"q": "YOKBÖYLE"})
        self.assertEqual(self._as_list(resp.data), [])

    def test_uye_q_rol_aksani(self):
        # rol adı da aranabilir ("ogrenci" ↔ "Öğrenci")
        resp = self.client.get("/api/uyeler/", {"q": "ogrenci"})
        self.assertEqual(len(self._as_list(resp.data)), 2)
...
class RulesUnitTests(TestCase):
    ...
    def setUp(self):
        ...
        self.rol = Rol.objects.create(ad="Öğrenci")
        self.uye = Uye.objects.create(
            ad="Ali", soyad="Veli", uye_no="60001", sinif=self.sinif, rol=self.rol
        )
    ...
    def test_student_pasif_sets_date_and_warns(self):
        self._loan()
        warnings = apply_student_status(self.uye, False)
        self.uye.refresh_from_db()
        self.assertFalse(self.uye.aktif)
        self.assertIsNotNone(self.uye.pasif_tarihi)
        self.assertTrue(any("aktif ödüncü var" in w for w in warnings))

    def test_student_back_active_clears_date(self):
        apply_student_status(self.uye, False)
        apply_student_status(self.uye, True)
        self.uye.refresh_from_db()
        self.assertTrue(self.uye.aktif)
        self.assertIsNone(self.uye.pasif_tarihi)

    def test_delete_rules(self):
        self.assertTrue(can_delete_uye(self.uye))
        self.assertTrue(can_delete_nusha(self.nusha))
        self.assertTrue(can_delete_kitap(self.kitap))
        self._loan()
        self.assertFalse(can_delete_uye(self.uye))
...
class LoanCloseAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.rol = Rol.objects.create(ad="Öğrenci")
        self.uye = Uye.objects.create(
            ad="Mehmet", soyad="Demir", uye_no="60124", sinif=self.sinif, rol=self.rol
        )
        self.kitap, self.nusha = make_book_and_copy(barkod="KIT0000777")

    def _checkout(self):
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60124", "barkod": "KIT0000777"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
...
class UyeDurumAPITests(APITestCase):
    """POST /api/uyeler/{id}/durum/ — aktif/pasif, yalnızca admin."""

    def setUp(self):
        make_policy()
        self.admin_user = User.objects.create_superuser(username="admin2", password="a2!")
        self.personel_user = User.objects.create_user(username="person2", password="p2!")
        self.sinif = Sinif.objects.create(ad="6-C")
        self.rol = Rol.objects.create(ad="Öğrenci")
        self.uye = Uye.objects.create(
            ad="Ayşe", soyad="Can", uye_no="60125", sinif=self.sinif, rol=self.rol
        )

    def test_non_admin_forbidden(self):
        self.client.force_authenticate(self.personel_user)
        resp = self.client.post(f"/api/uyeler/{self.uye.id}/durum/", {"aktif": False}, format="json")
    ...
    def test_admin_pasif_sets_date_and_active_clears(self):
        ...
        self.assertFalse(self.uye.aktif)
        self.assertIsNotNone(self.uye.pasif_tarihi)
        resp = self.client.post(f"/api/uyeler/{self.uye.id}/durum/", {"aktif": True}, format="json")
        self.uye.refresh_from_db()
        self.assertTrue(self.uye.aktif)
        self.assertIsNone(self.uye.pasif_tarihi)
    ...
    def test_admin_pasif_warns_active_loans(self):
        ...
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60125", "barkod": nusha.barkod}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
        resp = self.client.post(f"/api/uyeler/{self.uye.id}/durum/", {"aktif": False}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
...
class UyeCRUDAPITests(APITestCase):
    ...
    def test_personel_can_create_student(self):
        resp = self.client.post(
            "/api/uyeler/",
            {"ad": "Yeni", "soyad": "Öğrenci", "uye_no": "998877"},
            format="json",
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
    ...
    def test_create_duplicate_no_rejected(self):
        self.client.post(
            "/api/uyeler/",
            {"ad": "İlk", "soyad": "Öğrenci", "uye_no": "998877"},
            format="json",
        )
        resp = self.client.post(
            "/api/uyeler/",
            {"ad": "İkinci", "soyad": "Öğrenci", "uye_no": "998877"},
            format="json",
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_personel_can_edit_profile_fields(self):
        uye = Uye.objects.create(ad="Eski", soyad="Ad", uye_no="998877")
    ...
    def test_patch_aktif_locked_read_only(self):
        uye = Uye.objects.create(ad="A", soyad="B", uye_no="998877")
        resp = self.client.patch(f"/api/uyeler/{uye.id}/", {"aktif": False}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        uye.refresh_from_db()
        self.assertTrue(uye.aktif)

    def test_personel_cannot_delete(self):
        uye = Uye.objects.create(ad="A", soyad="B", uye_no="998877")
        resp = self.client.delete(f"/api/uyeler/{uye.id}/")
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Uye.objects.filter(pk=uye.pk).exists())
    ...
    def test_admin_can_delete_if_no_history(self):
        ...
        self.assertFalse(Uye.objects.filter(pk=uye.pk).exists())

    def test_admin_cannot_delete_student_with_history(self):
        self.client.force_authenticate(self.admin_user)
        uye = Uye.objects.create(ad="A", soyad="B", uye_no="60126")
        kitap, nusha = make_book_and_copy(barkod="KIT0000777")
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60126", "barkod": nusha.barkod}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
        resp = self.client.delete(f"/api/uyeler/{uye.id}/")
...
class KatalogAPITests(APITestCase):
    ...
    def test_durum_duzelt_oduncte_rejected(self):
        self.client.force_authenticate(self.admin)
        uye = Uye.objects.create(ad="A", soyad="B", uye_no="60127")
        kitap, nusha = make_book_and_copy(barkod="KIT0000707")
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60127", "barkod": "KIT0000707"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
        resp = self.client.post(
            f"/api/nushalar/{nusha.id}/durum_duzelt/", {"durum": "mevcut"}, format="json"
    ...
    def test_kitap_destroy_with_history_blocked(self):
        self.client.force_authenticate(self.admin)
        uye = Uye.objects.create(ad="A", soyad="B", uye_no="60128")
        kitap, nusha = make_book_and_copy(barkod="KIT0000713")
        resp = self.client.post(
            "/api/checkout/", {"uye_no": "60128", "barkod": "KIT0000713"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.content)
        resp = self.client.delete(f"/api/kitaplar/{kitap.id}/")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST, resp.content)
...
class BorrowerAccountTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.rol = Rol.objects.create(ad="Öğrenci")
        self.uye = Uye.objects.create(
            ad="Ali", soyad="Veli", uye_no="9001",
            sinif=self.sinif, rol=self.rol,
        )
        self.diger = Uye.objects.create(
            ad="Ayşe", soyad="Yılmaz", uye_no="9002",
            sinif=self.sinif, rol=self.rol,
        )
        self.admin = User.objects.create_superuser(
            username="admin", password="a1!"
        )

    def _set_password(self, uye, sifre):
        self.client.force_authenticate(self.admin)
        resp = self.client.patch(
            f"/api/uyeler/{uye.id}/", {"sifre": sifre}, format="json"
        )
    ...
    def test_uye_login_and_scope(self):
        resp = self._set_password(self.uye, "ilk1234")
        self.assertEqual(resp.status_code, status.HTTP_200_OK, resp.content)
        ...
        self.assertEqual(
            self.client.get("/api/uye-gecmis/9001/").status_code,
            status.HTTP_200_OK,
        )
        self.assertEqual(self.client.get("/api/uye-gecmis/9002/").data, [])
        self.assertEqual(
            self.client.get("/api/uye-ceza/9002/").status_code,
            status.HTTP_403_FORBIDDEN,
        )

    def test_uye_change_password_clears_flag(self):
        self._set_password(self.uye, "ilk1234")
        login = self._login("9001", "ilk1234")
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        resp = self.client.post(
            "/api/change-password/",
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK, resp.content)
        self.uye.refresh_from_db()
        self.assertFalse(self.uye.parola_degistirilsin)

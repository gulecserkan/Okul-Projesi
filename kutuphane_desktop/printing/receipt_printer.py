...
def _render_template_text(template_body: str, context: Dict[str, str]) -> str:
    def replacer(match):
        key = match.group(1)
...
def _configure_printer(printer: QPrinter, receipt_prefs: Dict[str, Any], printing_prefs: Dict[str, Any]) -> None:
...
def print_receipt_from_template(template_key: str, context: Dict[str, Any]) -> None:
    ...
    rendered = _render_template_text(body, _stringify_context(context))
    ...
    _configure_printer(printer, receipt_prefs, printing_prefs)
    ...
    font = QFont()
    font_size = int(receipt_prefs.get("font_pt", 10) or 10)
    font_family = receipt_prefs.get("font_family")
    if font_family:
        font.setFamily(font_family)
    font.setPointSize(font_size)
    doc.setDefaultFont(font)
    doc.setPlainText(rendered)
